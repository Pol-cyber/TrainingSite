...
public interface MUserToUserRepo extends CrudRepository<MessageUserToUser,Long> {
    ...
    @Transactional
    @Modifying
    @Query("UPDATE MessageUserToUser m SET m.isRead = true WHERE m.senderId = :senderId AND m.receiverId = :receiverId")
    void updateAllMessage(@Param("senderId") String senderId, @Param("receiverId") String receiverId);
